    ...
    #####        print record
        ...
        for pagenum in range(1):
            ...
            if test1:
                ...
                while len(test3) >= 5 and historycnt <= 3000:
                ...
                #print ASX_Code
                    ...
                    scraperwiki.sqlite.execute("insert or replace into Signal_History values (?, ?, ?, ?, ?, ?)",  [asxcode, sh_Date, sh_Price, sh_Signal, sh_Confirmation, sh_AUD100]) 
...
if 1==1:
    ...
    eoddate1 = soup.findAll("div", {"class": "header-timestamp"})[0].text[-11:].replace(" ", "-")
    date_obj = datetime.strptime(eoddate1, '%d-%b-%Y')
    eoddate1 = date_obj.strftime('%Y-%m-%d')
    #eoddateint = int(date_obj.strftime('%Y%m%d'))
    ...
    if eoddate1 <> eoddate:
        ...
        for sublst in output_rows:
            if len(sublst) > 0:
                ...
                scraperwiki.sqlite.execute("insert or ignore into company values (?, ?, ?, ?, ?, ?, ?, ?, ?)",  [rank, code, company, price, change, perchg, yrperchg, marketcap, eoddate1]) 
